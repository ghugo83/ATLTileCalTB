...
void MyFinalStateHistoManager::ScoreSecondary(const int secondaryIndex, 
  ...
  const auto found = fParticleData.find(secondaryIndex);
  ...
  // Otherwise, create histos for that particle.
	  ...
	  const G4String histoTitle = G4String("Particle pdg==" + particleName + " spectrum");
	  ...
	  fParticleData.insert(std::make_pair(secondaryIndex, std::move(histoWrapper)));
  }

  // Fill the G4H1Wrapper.
//const G4double kineticEnergy = secondary->GetKineticEnergy();
  particleHistoWrapper->Fill(score, 1.);
...
void MyFinalStateHistoManager::EndOfRun() const {
  ...
  for (const auto& particleIt : fParticleData) {
    const G4int particleIndex = particleIt.first;
    G4String particleName = "caca";
    if (particleIndex == 0) {
	    particleName = "total";
    }
	    ...
	    particleName = "pionPlus";
    }
    else if (particleIndex == 4) {
	    particleName = "gamma";
    }
    else if (particleIndex == 5) {
	    particleName = "electron";
    }
    else if (particleIndex == 6) {
	    particleName = "others";
    }
  ...
  G4cout << "fNumEvents = " << fNumEvents << G4endl;
